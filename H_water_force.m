function [phi_veritical,H,R]= H_water_force(h,v_wind,v_water,l_maolian,m_maolian_for_everymile,m_zhongwuqiu,l_total_maolian)
%添加水流力函数
%%%定义常量%%%%%%%%%%%%%%（使用SI制单位）
%重力加速度
g=9.8;
...
%水流大小
%v_water = 1.5
%水平方向，T1cos_theta = F_风 + 水流力
T_cos_theta = 0.625.*D * (v_wind)^2 + WaterForce(2*h,v_water);
...
T_cos_theta = T_cos_theta + WaterForce(l_gangguan * d_gangguan*sin(phi),v_water);
...
for i_bianli = 2 : n
    ...
    elseif(i_bianli == 5)
     ...
     T_cos_theta = T_cos_theta + WaterForce(1*0.3*sin(phi),v_water);
